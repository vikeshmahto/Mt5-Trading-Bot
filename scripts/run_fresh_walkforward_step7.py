"""
scripts/run_fresh_walkforward_step7.py
──────────────────────────────────────
Executes Step 7: Clean Walk-Forward on Fresh, Genuinely Unseen Data across All Instruments.

Date Ranges (100% Pristine, NEVER used in any prior debugging, in-sample, or holdout):
...
Features:
  - Market Regime filter active (suppresses entries during choppy regimes).
  - All automated invariant assertions strictly enforced in backtest/engine.py.
  - Full metrics per instrument, per regime, per setup type, and per score tier.
  - Daily-resampled Sharpe and Sortino ratios.
"""
...
log = get_logger("scripts.step7_walkforward")
...
def run_walkforward_slice(
    ...
    log.info("=" * 72)
    log.info(f">>> STEP 7 WALK-FORWARD: {symbol} [{label}] ({start_date} to {end_date})")
    log.info("=" * 72)
    ...
    m15_all = bars_dict.get("M15", pd.DataFrame())
    if m15_all.empty:
        log.error(f"No M15 bars found for {symbol}!")
        return None

    # Slicing M15
    mask = (m15_all.index >= start_date) & (m15_all.index <= end_date)
    m15_slice = m15_all[mask]
    ...
    # Save reports
    reports_dir = Path("reports/walkforward_step7")
    ...
    # Detailed Regime & Trade Attribution
    trades_df = pd.read_csv(csv_path) if os.path.exists(csv_path) and len(result.trades) > 0 else pd.DataFrame()

    print("\n" + "=" * 72)
    print(f"WALK-FORWARD PERFORMANCE REPORT: {symbol} - {label}")
    print("=" * 72)
    print(f"  Bars Evaluated       : {len(m15_slice)}")
...
def main():
    test_slices = [
        {
            ...
            "end_date": "2026-02-27",
            "label": "XAUUSD Winter 2025-2026 Walk-Forward",
            "output_prefix": "step7_wf_winter2025_2026",
        },
        {
            "symbol": "BTCUSD",
            "start_date": "2026-03-01",
            "end_date": "2026-05-31",
            "label": "BTCUSD Spring 2026 Walk-Forward",
            "output_prefix": "step7_wf_spring2026",
        },
        {
            "symbol": "EURUSD",
            "start_date": "2026-01-05",
            "end_date": "2026-04-30",
            "label": "EURUSD Q1 2026 Walk-Forward",
            "output_prefix": "step7_wf_q1_2026",
        },
    ]
    ...
    log.info("\n" + "=" * 72)
    log.info("ALL STEP 7 CLEAN WALK-FORWARD TESTS COMPLETED SUCCESSFULLY!")
    log.info("=" * 72)
